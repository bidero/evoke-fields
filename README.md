...
- **Poza indeksem** (checkbox per CPT i per taksonomia): typ znika z `wp-sitemap.xml`, dostaje `exclude_from_search` (wyszukiwarka WordPressa) i `<meta name="robots" content="noindex, follow">` na wpisie, archiwum typu i archiwum termu. Dla typów renderowanych wewnątrz innych stron — slidery, menu, listy referencji — których wpisy nie mają być osobnymi podstronami. To **nie** jest „Chroniony": nic nie zostaje zamknięte, wpis dalej renderuje się wszędzie tam, gdzie go wstawiono. Z zainstalowanym Evoke ONE (1.221.0+) flaga widoczna także w SEO → Mapa strony, jako zaznaczona i zablokowana — źródłem jest ten ekran.
